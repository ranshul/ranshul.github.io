...
On a related note, there some commands you can use to explore memory-related events.
...
Fine, but `calloc` is supposed to work! Turns out, `calloc` touches only a few page tables. The solution was to use a `memset` in addition which touches every page in the allocation. To check this hazy memory, I searched again and found an [answer](http://stackoverflow.com/questions/2688466/why-mallocmemset-is-slower-than-calloc) which had everything I needed. 

The final tester looks like this:
{% highlight python %}
#include <sys/mman.h>
#include <fcntl.h>
#include <sys/stat.h>
...
The whole ordeal lasted for 3 interesting hours.
